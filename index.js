window.onload = () => {
          ...
          },
          {
              ...
              episodeLinks: [
                  { start: 1,   end: 5,   url: `${dailymotionBaseUrl}x8rfvh7` }, // Uploader: Donghua Addict
                    ...
                    { start: 94,  end: 99,  url: `${dailymotionBaseUrl}x8rjc56` }, // Uploader: Donghua Addic
    ...
    function showSearchView(query, results) {
        ...
        if (results.length === 0 && searchResultsGrid) {
    ...
    function setActiveNavLink(activeLink) {
        [homeLink, moviesLink, mobileHomeLink, mobileMoviesLink].forEach(link => {
        ...
        if(activeLink === homeLink || activeLink === mobileHomeLink) {
            ...
            if (moviesLink) moviesLink.classList.add('active');
            if (mobileMoviesLink) mobileMoviesLink.classList.add('active');
        }
    }
    ...
    function renderEpisodeList() {
        ...
        if(showMoreContainer) {
            showMoreContainer.innerHTML = ''; // Clear show more button
        }
    ...
    function selectEpisode(episodeNumber) {
        ...
        const watchTitleHeader = document.getElementById('watch-title-header');
        if(watchTitleHeader) watchTitleHeader.textContent = episodeTitle;

        const watchMetadata = document.getElementById('watch-metadata');
        if(watchMetadata) {
        ...
        const videoDescText = document.getElementById('video-desc-text');
    ...
    // FIXED: This function now checks if the gridElement exists before trying to use it.
    function renderCards(items, gridElement, clear = false) {
        if (!gridElement) {
            console.error("Attempted to render cards into a non-existent element.");
            return;
        }
    ...
    // FIXED: This function now checks if the listElement exists before trying to use it.
    function renderList(items, listElement, type, clear = false) {
        if (!listElement) {
            console.error("Attempted to render a list into a non-existent element.");
            return;
        }
    ...
    function initializePage() {
        try {
            ...
            // FIXED: This block will run regardless of whether an error occurred or not,
            // ensuring the loading spinner is always hidden.
        ...
        // Navigation
        if (homeLink) homeLink.addEventListener('click', showLibraryPage);
        if (moviesLink) moviesLink.addEventListener('click', showMoviesPage);
        if (mobileHomeLink) {
